...
class Slider {
  ...
  void setIgnoreMouse (boolean b) {
   IGNORE_MOUSE = b; 
  }


void update(int sValue) {
  
        IGNORE_MOUSE = true;
        currentValue = (int) map (sValue, 0 , 255, sliderMin, sliderMax );
        currentValue = min (currentValue, sliderMax);
        currentValue = max (currentValue, sliderMin);

       
        activeValue = (int)map (currentValue, 0 , 255 , loc[1] + loc[3] , loc[1] );
